...
export function parseInitialUrlState(events: Event[]): {
  ...
  searchQuery: string;
  activeFilter?: 'All' | 'Los Angeles' | 'New York City' | 'Miami' | 'Austin' | 'Las Vegas' | 'Atlanta' | 'Houston' | 'Raleigh' | 'Dallas' | 'Salt Lake City';
} {
  try {
    ...
    if (cityParam) {
      const validCities = ['Los Angeles', 'New York City', 'Miami', 'Austin', 'Las Vegas', 'Atlanta', 'Houston', 'Raleigh', 'Dallas', 'Salt Lake City'] as const;
